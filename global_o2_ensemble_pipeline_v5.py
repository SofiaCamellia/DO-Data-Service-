...
# OUTPUT_ROOT = Path(r"E:\codex_code\ensemble5")
DATA_PATH = Path(r"/home/bingxing2/home/scx7l1f/IAP_TSDO.npy")
OUTPUT_ROOT = Path(r"/home/bingxing2/home/scx7l1f/rec/ensemble/world1")
...
def summarize_physics_violations(X, preds, config: RunConfig):
    ...
    return {
        ...
        "shallow": bucket(depth <= 50.0),
        "mid": bucket((depth > 50.0) & (depth <= 200.0)),
        "deep": bucket(depth > 200.0),
    }
...
def model_factories(config: RunConfig):
    ...
    factories = {
        ...
        "ert": lambda: ExtraTreesRegressor(
            n_estimators=200 if not smoke else forest_estimators,
            max_depth=24,
            min_samples_leaf=10,
            max_features=0.8,
            bootstrap=False,
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Global six-model stacking + PINN pipeline.")
    parser.add_argument("--dry-run", action="store_true", help="Load data, split by year 8:1:1, and save summary only.")
...
def main():
    ...
    X_clean, y_clean, keep_clean, raw_label_filter = filter_invalid_labels(X_all, y_all, config, "all_filtered")
    del keep_clean
    ...
    split_report = {
        ...
        "data_counts": {
            "all_filtered_before_label_clean": int(len(X_all)),
            "all_filtered_after_label_clean": int(len(X_clean)),
